...
BIC(mod2) # 1169.058
vif(mod2) # 1.247826
cases_model <- lm(sum_cases_p100k~rems+htt, data = cases_p_100k_svi)
...
summary(mod2)
BIC(mod2) # 1106
vaccine_model <- lm(vacs_100k~hhc+rems, vax_2021_clean)
...
ggPredict(cases_model, interactive = TRUE)
...
ggPredict(vaccine_model, interactive = TRUE)


# Nice Tables for Models

# Nice table for Cases Model


cases_table <- matrix(c('Intercept', 18786, 1509, 12.449, '<2e-16',
                      'rems', 11949, 2571, 4.647, 2.14e-05,
                      'htt', 5769, 2573, 2.242, 0.029),
                    nrow = 3, byrow = TRUE)
...
cases_tib |>
  ...
  tab_header(
    title = md('**Covid-19 Cases Model Summary**')
  ) |>
  tab_source_note(
    source_note = "Multiple R-squared: 0.4489"
  ) |> 
  tab_source_note(
    source_note = 'F-statistic: 22.4 on 2 and 55 DF'
  ) |> 
  tab_source_note(
    source_note = 'p-value: 7.641e-08'
  ) |>
  gt_theme_538()
...
# Nice Table for Vax Model

vac_table <- matrix(c('Intercept', 76385, 4637, 16.473, '<2e-16',
                      'hhc', -28901, 6875, -4.204, 0.000117,
                      'rems', 19984, 7382, 2.707, 0.009431),
                    nrow = 3, byrow = TRUE)
...
vac_tib |>
  gt(rowname_col = 'Independent Variable') |>
  tab_header(
    title = md('**Covid-19 Vaccinations Model Summary**')
  ) |>
  tab_source_note(
    source_note = "Multiple R-squared: 0.2898"
  ) |> 
  tab_source_note(
    source_note = 'F-statistic: 9.588 on 2 and 47 DF'
  ) |> 
  tab_source_note(
    source_note = 'p-value: 0.0003221'
  ) |>
  gt_theme_538()
