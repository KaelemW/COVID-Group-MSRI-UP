...
correlations <- matrix(c('svi', 0.5311, 0.0000179,
                         'rems', 0.6313, 0.0000001077,
                         'htt', 0.4822, 0.0001265,
                         'hhc', 0.3263, 0.01243,
                         'ses', 0.4311, 0.00007289),
                       nrow = 5, byrow = TRUE)
colnames(correlations) <- c('Variable', 'Correlation', 'Pr(>|t|)')

cor_tib <- as_tibble(correlations)


cor_tib$`Pr(>|t|)` <- as.numeric(cor_tib$`Pr(>|t|)`)
color_palette <- c("darkgreen", "white")

cor_tib |>
  gt(rowname_col = 'Variable') |>
  tab_header(
    title = md('**Covid-19 Cases Correlation Summary**')
  ) |>
  gt_theme_538() |>
  gt_color_rows(
    columns = 3, 
...
# for vax
correlations <- matrix(c('svi', -0.3010, 0.03367,
                         'rems', 0.1508, 0.29,
                         'htt', 0.0763, 0.6,
                         'hhc', -0.4231, 0.002204,
                         'ses', -0.3622, 0.009749),
...
cor_tib |>
  gt(rowname_col = 'Variable') |>
  tab_header(
    title = md('**Covid-19 Vaccinations Correlation Summary**')
  ) |>
  gt_theme_538() |>
  gt_color_rows(
    columns = 3, 
